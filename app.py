...
# 左侧参数选择
feature1 = st.sidebar.selectbox("Gene", ["CD3D", "CD3E","CD4","CD8A", "CD14","CD68","CD79A","CLEC10A","COL1A1","CSF3R","DCN","FAP","FOXP3","IGHG1","IGKC","JCHAIN","KRT8","KRT18","KRT19","NKG7","TPSB2","VWF","EPCAM"])
...
if submit:
    image_path = f"images/{feature2}.png"
    if os.path.exists(image_path):
        st.image(Image.open(image_path), caption=f"{feature1}", use_container_width=True)
    else:
        st.warning("找不到对应的图片，请确认参数组合和文件名是否一致。")
